...
function render(txt, tipo) {
    var obras = obtener('obras');
    var resultado = [];

    for (var i = 0; i < obras.length; i++) {
        if (obras[i].estado !== 'publicada') continue;
        if (txt !== '' && obras[i].titulo.toLowerCase().indexOf(txt.toLowerCase()) === -1) continue;
        if (tipo !== 'todos' && obras[i].tipo !== tipo) continue;
        resultado.push(obras[i]);
    }

    var contenedor = document.getElementById('a7');
    var mensaje = document.getElementById('a8');
...
window.onload = function () {
    ...
    var campoBuscar = document.getElementById('a5');
    var filtroTipo = document.getElementById('a6');

    campoBuscar.oninput = function () {
        render(campoBuscar.value, filtroTipo.value);
    };
